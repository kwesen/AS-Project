...
```
```{r}
eta.table <- data.frame(Value = eta.sqr, Effect_size = character(length(eta.sqr)))

for (i in seq_along(eta.sqr))
{
  if (eta.sqr[i]<=0.01)
  {
    eta.table$Effect_size[i] <- "Neglegible"
  }
  else if (eta.sqr[i]>0.01 && eta.sqr[i] <= 0.06){
    eta.table$Effect_size[i] <- "Small"
  }
  else if (eta.sqr[i]>0.06 && eta.sqr[i] <= 0.14){
    eta.table$Effect_size[i] <- "Medium"
  }
  else 
  {
    eta.table$Effect_size[i] <- "Large"
  }
}
