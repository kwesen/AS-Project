...
for (i in seq_along(eta.sqr))
{
  if (eta.sqr[i]<=0.01)
  {
    eta.table$Magnitude[i] <- "Neglegible"
  }
...
conover.compare <- function(conover_processed, confidence = NULL)
{
  ...
  # }
return(as.data.frame(conover.comparison))
}
...
cliff.delta.for_conover <- function(temp_mat){
  ...
  column_names <- colnames(temp_mat)[-ncol(temp_mat)]
